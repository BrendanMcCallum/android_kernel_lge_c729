...
#define HS_STEREO_NOMIC_HEADSET_K	0x83
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_STEREO_NOMIC_HEADSET_K, SW_HEADPHONE_INSERT),
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	printk(KERN_INFO "%s : key=0x%x key_code=0x%x",__func__,key,key_code);
	...
	case KEY_VOLUMEDOWN:
		...
		if(key == KEY_MEDIA)
		{
			wake_lock_timeout(&headset_wake_lock, 5*HZ);
		}
	...
	case SW_HEADPHONE_INSERT_W_MIC:
		...
		input_report_switch(hs->ipdev, SW_MICROPHONE_INSERT,
							hs->mic_on);
		update_state();
		// prevent to enter suspend when insert/eject headset
	...
	case SW_HEADPHONE_INSERT:
		...
		//update_state();
		if(key_code == HS_STEREO_NOMIC_HEADSET_K)
		{
			hs->hs_on = 1;
			input_report_switch(hs->ipdev, key, hs->hs_on);
			update_state();
		...
		else if(key_code == HS_STEREO_HEADSET_K)
		{
		...
		else if(key_code == HS_REL_K)
		{
			...
			switch_set_state(&(hs->sdev), NO_DEVICE);
			update_state();
			// prevent to enter suspend when insert/eject headset
			wake_lock_timeout(&headset_wake_lock, 5*HZ);
		}
...
static ssize_t msm_headset_print_name(struct switch_dev *sdev, char *buf)
{
	...
	case MSM_NO_MIC_HEADSET:
		return sprintf(buf, "No_Mic_Headset\n");
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&headset_wake_lock, WAKE_LOCK_SUSPEND, "rpc_headset");
