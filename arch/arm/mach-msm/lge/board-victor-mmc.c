...
static struct mmc_platform_data bcm432x_sdcc_wlan_data = {
    ...
    .msmsdcc_fmax	= 24576000, // 49152000,
    .nonremovable	= 1,
};
#endif   
...
#if defined(LGE_MODEL_E739) 
#if defined(LGE_MODEL_E739_EVB_2) 
	vreg_mmc = vreg_get(NULL, "gp4");	// kh.tak MicroSD Detection
#else 
	vreg_mmc = vreg_get(NULL, "gp10");	// kh.tak MicroSD Detection
#endif // LGE_MODEL_E739_EVB_2
#else // Victor Original
	vreg_mmc = vreg_get(NULL, "gp4");	// kh.tak MicroSD Detection
#endif	// #if defined(LGE_MODEL_E739) 
	if (IS_ERR(vreg_mmc)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
