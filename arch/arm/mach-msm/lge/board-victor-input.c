 ...
 * skl6100 maps buttons at s1,s2,s3(menu,home,back)
 */
...
{
    touch_panel_i2c_device.id = 0;

	main_ts_set_vreg(1);

	init_gpio_i2c_pin(&touch_panel_i2c_pdata, touch_panel_i2c_pin[0], &touch_panel_i2c_bdinfo[0]);

	/* Fix Bus Num to 0, Because Touch SCL, SDA Use H/W I2C */
	i2c_register_board_info(0, &touch_panel_i2c_bdinfo[0], 1);

	platform_device_register(&touch_panel_i2c_device);
}
