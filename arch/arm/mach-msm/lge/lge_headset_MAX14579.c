...
static void remove_headset(void)
{
	...
	if (atomic_read(&hi->btn_state) == BUTTON_PRESSED)
		button_released();
	atomic_set(&hi->btn_state, BUTTON_RELEASED);
...
static void hs_detect_work_func(struct work_struct *work)
{
	...
	/* there is no change in headset status*/
...
{
	int state = gpio_get_value(hi->hs_detect_gpio);
...
static irqreturn_t hook_switch_irq_handler(int irq, void *dev_id)
{
	...
	/* skip if button status is same */
	/* if (atomic_read(&hi->btn_state)^value) */
...
{
	int ret;
	struct vreg *gp4_vreg = vreg_get(0, "xo_out");

	/* Enable Power of headset det IC */
	vreg_set_level(gp4_vreg, 2600);
	...
	hi->hs_mode_gpio = 31;
	hi->hsd_debounce_time = 500; /* 500 ms */
	...
	ret = set_irq_wake(hi->hook_switch_irq, 1);
	if (ret < 0) {
		HSD_ERR("Failed to set interrupt wake\n");
		goto err_request_input_dev;
	}
