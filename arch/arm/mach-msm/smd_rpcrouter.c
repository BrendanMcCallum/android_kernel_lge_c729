...
// LGE_UPDATE_E	
	...
	for(;;) {
		...
		if(rr_closed == 1)
		{
			printk(KERN_ERR "rpcrouter: rpc router was already closed. ignore rr_read!\n");
			return 0;
		}
#endif
...
#ifdef CONFIG_LGE_RPC_DOG_CRASH_DEBUG
	char currenttask_name[50];
#endif
 

 
#ifndef CONFIG_LGE_RPC_DOG_CRASH_DEBUG
	IO("READ on ept %p\n", ept);
#endif
...
#ifdef CONFIG_LGE_RPC_DOG_CRASH_DEBUG
	IO("READ on ept %p from %s\n", ept, get_task_comm(currenttask_name, current));
#endif
// LGE_UPDATE_E
...
#ifdef CONFIG_LGE_RPC_DOG_CRASH_DEBUG
	IO("READ on ept %p (%d bytes) from %s\n", ept, rc, get_task_comm(currenttask_name, current));
#else
