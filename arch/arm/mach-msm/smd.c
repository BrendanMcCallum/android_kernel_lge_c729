...
// LGE_UPDATE_S rpc_stuck
#ifdef CONFIG_LGE_RPC_DOG_CRASH_DEBUG_SMEM
typedef struct irqsoff_debug {
	unsigned int last_irq_off_pid;
	unsigned long last_irq_clock;
	unsigned long last_irq_ip;
	unsigned long last_irq_p_ip;
} irqsoff_debug_type;

typedef struct irq_stat {
	unsigned long sec;
	unsigned long usec;
	int cpu;
	int irq;
...
typedef struct ScheduleLogData {
	unsigned long ulTick;
	unsigned long ulMsec;
	unsigned int unPid;
	unsigned char ucComm[16];
} ScheduleLogData_type;

void* smem_ap_stuck_debug_ptr = NULL;
irqsoff_debug_type* smem_irqsoff_debug_ptr = NULL;
irq_stat_type* smem_irq_state_debug_ptr = NULL;
ScheduleLogData_type* smem_sched_debug_ptr = NULL;

void* smem_ap_stuck_debug_ptr;
#endif
// LGE_UPDATE_S
...
// LGE_UPDATE_S rpc_stuck
#ifdef CONFIG_LGE_RPC_DOG_CRASH_DEBUG_SMEM
	smem_ap_stuck_debug_ptr = smem_alloc2(SMEM_ID_VENDOR0, sizeof(irqsoff_debug_type)+sizeof(irq_stat_type)+sizeof(ScheduleLogData_type));

	smem_irqsoff_debug_ptr = (irqsoff_debug_type*)smem_ap_stuck_debug_ptr;
	smem_irq_state_debug_ptr = (irq_stat_type*)(int)smem_ap_stuck_debug_ptr+sizeof(irqsoff_debug_type);
	smem_sched_debug_ptr = (ScheduleLogData_type*)(int)smem_irq_state_debug_ptr+sizeof(ScheduleLogData_type);
#endif	
// LGE_UPDATE_E
...
{
	if(smem_irqsoff_debug_ptr != NULL)
	{
		smem_irqsoff_debug_ptr->last_irq_off_pid = last_irq_off_pid;
		smem_irqsoff_debug_ptr->last_irq_clock = last_irq_clock;
		smem_irqsoff_debug_ptr->last_irq_ip = last_irq_ip;
		smem_irqsoff_debug_ptr->last_irq_p_ip = last_irq_p_ip;
	}
}
EXPORT_SYMBOL(smem_irqsoff_debug_log);

void smem_irq_state_debug_log
(
	unsigned long sec,
	unsigned long usec,
	int cpu,
...
{
	if(smem_irq_state_debug_ptr != NULL)
	{
		smem_irq_state_debug_ptr->sec = sec;
		smem_irq_state_debug_ptr->usec = usec;
		smem_irq_state_debug_ptr->cpu = cpu;
		smem_irq_state_debug_ptr->irq = irq;
		smem_irq_state_debug_ptr->handler = handler;

	}
}
EXPORT_SYMBOL(smem_irq_state_debug_log);

void smem_sched_debug_log
(
	unsigned long ulTick,
	unsigned long ulMsec,
	unsigned int unPid,
...
{
	if(smem_sched_debug_ptr != NULL)
	{
		smem_sched_debug_ptr->ulTick = ulTick;
		smem_sched_debug_ptr->ulMsec = ulMsec;
		smem_sched_debug_ptr->unPid = unPid;
		memcpy(smem_sched_debug_ptr->ucComm,ucComm,16);
	}
}
EXPORT_SYMBOL(smem_sched_debug_log);
