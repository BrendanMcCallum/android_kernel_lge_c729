...
#include <asm/cacheflush.h>  //ADD
#endif

#ifdef CONFIG_MACH_LGE
// Reduce Audio AMP pop-noise
...
#ifdef CONFIG_WORKAROUND_RESTART_KERNEL_PANIC
static uint32_t last_restart_reason;
static uint32_t last_power_off;
#endif
...
{
#ifdef CONFIG_WORKAROUND_RESTART_KERNEL_PANIC
	// todo : restart reason이 일반 restart이었늗네. 다시 crash로 넘어오면 그냥 reboot으로 처리하자..
    // todo : power down 내려가는데 crash로 오면 power down 시켜버리자.
	last_power_off = 0x18182180;
#endif
...
#ifdef CONFIG_MACH_LGE
    /* disable irq for forbid task scheduling
     */
	local_irq_disable();
#endif
...
#define RECOVERY_REBOOT 0x77665502
#define ANDROID_RESTART 0xA1B2C3D4
#define POWER_OFF_REQ   0x6f686666
#endif
	...
	// todo : restart reason이 일반 restart이었늗네. 다시 crash로 넘어오면 그냥 reboot으로 처리하자..
    // todo : power down 내려가는데 crash로 오면 power down 시켜버리자.
	if(restart_reason == ANDROID_RESTART || restart_reason == RECOVERY_REBOOT)
	{
    ...
    {
        unsigned cmd = 2; /* CUSTOMER_CMD1_HW_RESET_IMM */;

#ifdef CONFIG_WORKAROUND_RESTART_KERNEL_PANIC
		if(last_power_off == 0x18182180)
		{
			printk(KERN_EMERG "msm_restart: last power off reqeust is set\n");
			printk(KERN_EMERG "             but, current request is kernel crash! forced power off\n");
		...
		if (last_restart_reason == ANDROID_RESTART || last_restart_reason == RECOVERY_REBOOT)
		{
			printk(KERN_EMERG "msm_restart: last restart reqeust is %x!\n",last_restart_reason);
			printk(KERN_EMERG "             but, current request is kernel crash! forced restart\n");
		...
		msm_proc_comm(PCOM_CUSTOMER_CMD1, &restart_reason, &cmd);
    }
	else if (restart_reason == RECOVERY_REBOOT)
	{
		unsigned cmd = 2; /* CUSTOMER_CMD1_HW_RESET_IMM */;
	...
	else if (restart_reason == POWER_OFF_REQ)
	{
		unsigned cmd = 7; /* CUSTOMER_CMD1_HW_POWEROFF_IMM */;
...
#ifdef CONFIG_MACH_LGE
		else if (!strcmp(cmd, "recovery_completed")) {
			restart_reason = 0x77665579;
		}
		else if (!strcmp(cmd, "power_off")) {
			restart_reason = 0x6f686666;
		}
#endif 
