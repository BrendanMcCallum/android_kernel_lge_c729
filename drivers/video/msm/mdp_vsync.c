...
//sumeet.gupta 040411 ONE_CODE : vsync on primary AND secondary LCDs.	
//	if ((mfd->dest != DISPLAY_LCD) || (mfd->panel_info.pdest != DISPLAY_1)
	if ((mfd->dest != DISPLAY_LCD) 
	    || (!vsync_mode)) {
	   printk("%s: Ignoring vsync for this lcd\n", __func__);
		goto err_handle;
	}
...
//]sumeet.gupta 040411 ONE_CODE
	if (mfd->panel_info.lcd.vsync_enable) {
		...
		if (mfd->use_mdp_vsync) {
			...
			printk(KERN_ERR "vsync check 2\n");

			if (mdp_vsync_clk_speed_hz == 0) {
				mfd->use_mdp_vsync = 0;
				printk(KERN_ERR "vsync check 3\n");
			} else {
				printk(KERN_ERR "vsync check 4\n");
				/*
