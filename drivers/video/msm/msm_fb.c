...
/* Sumeet 201210: DPSV framebuffer cannot be exposed through sysfs due to custom implementation */
#ifndef CONFIG_FB_MSM_DPSV
...
//[sumeet.gupta 040411 ONE_CODE
#ifdef CONFIG_FB_MSM_DPSV
/* Sumeet 050111: Restricting DPSV to just MDDI and EBI2 panels; the DTV panel should not go to DPSV.
 * However, it is _assumed_ that DTV panel's module probe happens _after_ both main and sub display panels;
 * else, DTV panel acquires fb0 !! This assumption is irrespective of DPSV or non-DPSV architecture.
*/
	if (mfd->panel_info.type != MDDI_PANEL && mfd->panel_info.type != EBI2_PANEL) {
	fbram_offset = PAGE_ALIGN((int)fbram)-(int)fbram;
	fbram += fbram_offset;
	fbram_phys += fbram_offset;
	fbram_size -= fbram_offset;
...
//[sumeet.gupta 040411 ONE_CODE
#ifdef CONFIG_FB_MSM_DPSV
	}else{ //not MDDI or EBI2 panel.
		if (register_dpsv_framebuffer(fbi) < 0) {
			if (mfd->lut_update)
				fb_dealloc_cmap(&fbi->cmap);
			      ...
			      struct fb_info *info)
{
	...
	if (start_drawing_flag)
	{
		printk("%s : display rendering start!",__func__);
		start_drawing_flag = 0;	
	}
