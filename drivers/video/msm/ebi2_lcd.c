...
static struct platform_driver ebi2_lcd_driver = {
	...
	.suspend = NULL,
//sumeet 171110 Old kernel version; not compatible with newer kernels.
#if 0
	.suspend_late = NULL,
	.resume_early = NULL,
#endif
...
static int ebi2_lcd_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
		...
		//outp32(ebi2_lcd_cfg1, 0x40A84084);
		//printk("ebi2_base:%p,%p,%p\n", ebi2_base, lcd01_base, lcd02_base);
		return 0;
	}
...
//sumeet: 171110 FLIP EVB has 1 EBI2 device, on secondary channel (CS5); so, changing as below:
//#ifdef CONFIG_MACH_LGE_FLIP  //SUG START
#ifdef LGE_MODEL_C729 //SUG START

		outp32(ebi2_base, (inp32(ebi2_base)&(~(EBI2_SECD_LCD_CLR)))
					|EBI2_SECD_LCD_SEL); 	//TBD: Check with T32: should be 0x901.
		outp32(ebi2_lcd_cfg0, mfd->panel_info.wait_cycle);
		//outp32(ebi2_lcd_cfg1, 0x40A84108 | (BIT(10) | BIT(24))); 	//from modem code.
		outp32(ebi2_lcd_cfg1, 0x40A84108); 	//from modem code.
		//outp32(ebi2_lcd_cfg1, 0x0); 	//from pecan code.
#else
...
//sumeet 171110 PM_RUNTIME not set => compile error!
