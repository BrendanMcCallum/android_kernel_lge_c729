...
//	printk("[QCTK_GAIN].mt9p017_write_exp_gain Func. Gain = %d, Line = %d\n", gain, line);
	...
	if(gain_NR < 0xD20 && line < 800)	//under 5.2
	{
		...
		rc = mt9p017_i2c_write_w_sensor(0x3102, 0x25);	//01

              rc = mt9p017_i2c_write_w_sensor(0x3F06, 0x00F0);	
	
		CDBG("[NOISE] under 5.2 : Outdoor..\n");			
	}
	else if(gain_NR < 0xD20) //5.2. 	//Indoor
	{
		rc = mt9p017_i2c_write_w_sensor(0x3100, 0x0002);
		rc = mt9p017_i2c_write_w_sensor(0x3102, 0x50);	//28

              rc = mt9p017_i2c_write_w_sensor(0x3F06, 0x00F0);	

		CDBG("[NOISE] under 5.2...\n");					
	}	
	...
	case UPDATE_PERIODIC:
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			if (rt == RES_PREVIEW)
			{
				...
				if(lge_get_hw_rev() < LGE_REV_10){
					rc = mt9p017_i2c_write_w_table(mt9p017_regs.prev_tbl_before,
					mt9p017_regs.prevtbl_size_before);
				}
				else{
			...
			if (rc < 0)
				...
				if(lge_get_hw_rev() < LGE_REV_10){
					rc = mt9p017_i2c_write_w_table(mt9p017_regs.snap_tbl_before,
					mt9p017_regs.snaptbl_size_before);
				}
				else{
...
int mt9p017_sensor_config(void __user *argp)
{
	...
	cdata.cfgtype);
		...
		case CFG_HW_REV:
			cdata.hw_rev = (unsigned char) lge_get_hw_rev();
			if (copy_to_user((void *)argp,
				&cdata,
				sizeof(struct sensor_cfg_data)))
				rc = -EFAULT;
