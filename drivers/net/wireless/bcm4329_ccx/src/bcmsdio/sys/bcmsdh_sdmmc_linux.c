...
#if defined(CONFIG_LGE_BCM432X_PATCH) /* dtim change for suspend current */
extern int dhdsdio_set_dtim(void *bus, int enalbe);
#endif /* dtim change for suspend current */
...
/*	ret = gpio_request(dhd_wifi_sleep->host_wake, "wifi_hostwakeup");
	if (ret < 0) {
		printk(KERN_ERR"[WiFi] Failed to get gpio_request \n");
		gpio_free(dhd_wifi_sleep->host_wake);
		return 0;
	}*/ // jisung.yang, temparary block to prevent lockup
...
dhd_unregister_hwakeup(void)
{
	dhd_disable_hwakeup();
	free_irq(dhd_wifi_sleep->host_wake_irq, NULL);
/*	gpio_free(dhd_wifi_sleep->host_wake); */ // jisung.yang, temparary block to prevent lockup
