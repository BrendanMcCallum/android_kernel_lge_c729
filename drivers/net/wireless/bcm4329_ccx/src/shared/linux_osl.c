...
typedef struct bcm_static_pkt {
 ...
 /*	struct sk_buff *skb_8k[MAX_STATIC_PKT_NUM];		<original> */
...
osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	if (!bcm_static_buf) {
		if (!(bcm_static_buf = (bcm_static_buf_t *)dhd_os_prealloc(3, STATIC_BUF_SIZE+
            ...
            return NULL; 						/* BRCM_UPDATE      <2011-0627, modify> */
		}
		else
...
//original	if (len > (PAGE_SIZE*2))
	...
	if (i != MAX_STATIC_PKT_NUM)
	{
		...
		up(&bcm_static_skb->osl_pkt_sem);
//original		skb = bcm_static_skb->skb_8k[i];
...
//origianl	for (i = 0; i < MAX_STATIC_PKT_NUM*2; i++)
	for (i = 0; i < MAX_STATIC_PKT_NUM; i++)
	{
