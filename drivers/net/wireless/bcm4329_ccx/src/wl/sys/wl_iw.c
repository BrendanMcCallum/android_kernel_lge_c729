...
int wl_iw_set_dtim_val(struct net_device *dev)
{
	...
	//	if (dtoh32(bi->version) == WL_BSS_INFO_VERSION )
		if (dtoh32(bi->version) == WL_BSS_INFO_VERSION && (!dlna_dtim_val))
...

		{
			...
			{
				wl_dtim_val = 3;
				printk("[%s]wl dtim val set : %d\n",__FUNCTION__, wl_dtim_val);
			}
...
)
{
	...
	if(g_onoff == G_WLAN_SET_ON){
        ...
        { 
            ...
            if( (arp_ret= dev_wlc_ioctl(dev, WLC_SET_VAR, buf, len)) < 0) 
                printk(KERN_ERR "%s:arp_hostip_clear failed.ret[%d]\n", __FUNCTION__, arp_ret); 
        } while(0); 
...
static void wl_iw_send_scan_complete(iscan_info_t *iscan)
{
	...
	char extra[IW_CUSTOM_MAX + 1];
		...
		wireless_send_event(iscan->dev, SIOCGIWSCAN, &wrqu, extra);  // 2011-0614, add
...
)
{
	...
	printk("[hyeok-debug] %s: wl_iw_get_cckm_rn\n", dev->name);

	if ((error = dev_wlc_intvar_get(dev, "cckm_rn", &rn)))
	{
		printk("[hyeok-debug] +++++++++++  %s: error [%d]\n",__func__,error);
		return error;
	}

	memcpy(extra, &rn, sizeof(u32));
	printk("[hyeok-debug] +++++++++++  %s: rn [%d]\n",__func__,rn);
...
/* only if receiving regular START command */
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			printk("%s:%d Before wl_iw_get_rssi\n",__func__,__LINE__);
			ret = wl_iw_get_rssi(dev, info, (union iwreq_data *)dwrq, extra);
			printk("%s:%d After  wl_iw_get_rssi\n",__func__,__LINE__);
		}
	...
	dlna_dtim_val = wl_dtim_val;
	/* not connected BSS. */
	if(dlna_dtim_val == 0)
		dlna_dtim_val = 1;
	wl_dtim_val = 1;
   } else if (strnicmp(extra, "DLNA-STOP", 9) == 0 ) {
   	wl_dtim_val = dlna_dtim_val;
   	dlna_dtim_val = 0;
	    ...
	    else if (strnicmp(extra, "get cckm_rn", strlen("get cckm_rn")) == 0)
		{
			...
			printk("[hyeok-debug] %s dwrq->length[%d]\n",__func__,dwrq->length);
		}
