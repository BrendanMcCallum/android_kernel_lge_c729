...
#include <linux/akm8975.h>	// akm daemon ioctl set
...
#define CTRL_REG5		0x24	/* sleep control reg */
...
/* kwangdo.yi@lge.com 10.09.16 S
   fix power on/off control register from 4->1
   */
...
static ssize_t show_kr3dh_sensor_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t store_kr3dh_sensor_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kr3dh_data *kr = i2c_get_clientdata(client);
	int mode=0;
...
static ssize_t show_kr3dh_sensor_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kr3dh_data *kr = i2c_get_clientdata(client);
	int xyz[3] = {0,};
...
static DEVICE_ATTR(kr3dh_sensor_enable, S_IRUGO | S_IWUSR, show_kr3dh_sensor_enable, store_kr3dh_sensor_enable);
static DEVICE_ATTR(kr3dh_sensor_data, S_IRUGO, show_kr3dh_sensor_data, NULL);

static struct attribute *kr3dh_attributes[] = {
	&dev_attr_kr3dh_sensor_enable.attr,
	&dev_attr_kr3dh_sensor_data.attr,
	NULL,
};
			   ...
			   const struct i2c_device_id *id)
{
	...
	/* kwangdo.yi 10.07.20
   add for check message
   */
#ifdef CONFIG_MACH_MSM8X55_FLIP
//#if defined(LGE_MODEL_C729_REV_EVB)

        printk("### kr3dh i2c->addr %x\n", client->addr);

        err = i2c_smbus_read_byte_data(client, 0x0F);
        if (err < 0)
                printk("### i2c error %d\n", err);
        else
                printk("### i2c read who_am_i : 0x%x\n", err);
//#endif
#endif
	...
	err = sysfs_create_group(&client->dev.kobj, &kr3dh_attribute_group);
	if (err) {
		printk(KERN_ERR "kr3dh sysfs register failed\n");
		goto err5;
	}
	
/* kwangdo.yi@lge.com 10.09.16 S
   initially turn on motion sensor
   */
...
err5:
	sysfs_remove_group(&client->dev.kobj, &kr3dh_attribute_group);
err4:
...
static int kr3dh_resume(struct i2c_client *client)
{
	...
	kr->resume_state[0] = 0x27;
	kr->resume_state[1] = 0x0;
	kr->resume_state[2] = 0x0;
	kr->resume_state[3] = 0x0;
	kr->resume_state[4] = 0x0;
...
{
	pr_info(KERN_INFO "KR3DH accelerometer driver\n");
	printk("### KR3DH accelerometer driver\n");
