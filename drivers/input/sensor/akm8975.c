...
#define AKM8975_POWEROFF //cg.kim AKM8975 power off for current
...
#ifdef AKM8975_POWEROFF
//static int powerDownOrOff=0; //For used current //diyu@lge.com
static int akm8975_set_vreg_check=  0;
#endif
 ...
 static int AKECS_PowerOff(void)
{
	...
	akm8975_pdata->power(0);
	akm8975_set_vreg_check=0;
...
//			break;
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GET_DELAY:
			...
			break;
 	case ECS_IOCTL_GET_ACCEL_PATH:
		sprintf(accel_dev_path, "/dev/%s", akm8975_pdata->accelerator_name);
		if (copy_to_user(argp, accel_dev_path, sizeof(accel_dev_path)))
			return -EFAULT;
		break;
	case ECS_IOCTL_ACCEL_INIT:
		fdata_sign[0] = akm8975_pdata->fdata_sign_x;
		fdata_sign[1] = akm8975_pdata->fdata_sign_y;
		fdata_sign[2] = akm8975_pdata->fdata_sign_z;
		fdata_sign[3] = akm8975_pdata->fdata_order0;
		fdata_sign[4] = akm8975_pdata->fdata_order1;
		fdata_sign[5] = akm8975_pdata->fdata_order2;
		fdata_sign[6] = akm8975_pdata->sensitivity1g;
...
#if defined(LGE_MODEL_C729_REV_EVB) // cg.kim@lge.com NO COM_INT GPIO for eCompass	
...
#ifdef AKM8975_POWEROFF
	akm8975_pdata->power(1);
	akm8975_set_vreg_check = 1;
#endif
...
static ssize_t show_akm8975_sensor_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	char strbuf[100];

	sprintf(strbuf, "%d", (atomic_read(&open_flag)));
	
	return sprintf(buf, "%s\n", strbuf);
}

static ssize_t store_akm8975_sensor_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int mode=0;
...
static ssize_t show_akm8975_sensor_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	char strbuf[100];

	printk("show_akm8975_sensor_data()");
	
	sprintf(strbuf, "%d %d %d", YPR_value[0], YPR_value[1], YPR_value[2]);

	return sprintf(buf, "%s\n", strbuf);
}

static DEVICE_ATTR(akm8975_sensor_enable, S_IRUGO | S_IWUSR, show_akm8975_sensor_enable, store_akm8975_sensor_enable);
static DEVICE_ATTR(akm8975_sensor_data, S_IRUGO, show_akm8975_sensor_data, NULL);

static struct attribute *akm8975_attributes[] = {
	&dev_attr_akm8975_sensor_enable.attr,
	&dev_attr_akm8975_sensor_data.attr,
	NULL,
};
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	akm8975_pdata = client->dev.platform_data;
	this_client = client;
...
//#endif
	...
	err = sysfs_create_group(&client->dev.kobj, &akm8975_attribute_group);
	if (err) {
		printk(KERN_ERR "akm8975 sysfs register failed\n");
		goto exit9;
	}
...
exit9:
	sysfs_remove_group(&client->dev.kobj, &akm8975_attribute_group);
exit8:
