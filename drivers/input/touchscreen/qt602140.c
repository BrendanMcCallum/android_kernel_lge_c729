...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.atchcalsthr = 5; //23;//35;
...
static unsigned int qt_success_flag=0;
/*
...
void check_chip_calibration(void)
{
	...
	if(tsp_version >=0x16)
	{
		...
		if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
			...
			/* process counters and decide if we must re-calibrate or if cal was good */      
			if (tch_ch > 0 && tch_ch <= 12 && atch_ch == 0) {
				/* cal was good - don't need to check any more */
				//if(!check_abs_time())
				//	qt_time_diff=301;	
				
				if(qt_timer_state == 1) {
					if( qt_success_flag >= 3/*qt_time_diff > 300*/) {
					   dprintk("[TSP] calibration was good\n");
					   printk("[TSP] calibration was good\n");
					   printk("[TSP] calibration was good value : %d\n", qt_success_flag);
			...
			{
				printk("[TSP] calibration was not decided yet2\n");
			}
			else {
...
//									up_check_bit=KEY_SEARCH;
	...
	if(touch_message_flag && (cal_check_flag)) {
		printk("check_chip_calibration enter\n");
